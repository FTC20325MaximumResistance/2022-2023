...
public class RobotArm extends Thread{
    ...
    public void run(){
        ...
        r.arm1.setPower(0.8);
        r.arm2.setPower(0.9);
        while ((!r.getTolerance(r.arm1.getCurrentPosition(), r.arm1.getTargetPosition(), 50) || !r.getTolerance(r.arm2.getCurrentPosition(), r.arm2.getTargetPosition(), 50)) && !opMode.isStopRequested()){
            if (r.getTolerance(r.arm1.getCurrentPosition(), r.arm1.getTargetPosition(), 50)){
                r.arm1.setPower(0);
            }
            if (r.getTolerance(r.arm2.getCurrentPosition(), r.arm2.getTargetPosition(), 50)){
                r.arm2.setPower(0);
            }
        }
